...
import { SentimentResult, TextStatistics, CommentAnalysis, AnalysisSummary, RequestBody, ApiErrorResponse, AnalyzedCommentsApiResponse } from '@/types/api/ai/sentiment-analysis';
...
function getTextStatistics(text: string): TextStatistics {
   ...
   return {
      ...
      averageWordsPerSentence: sentences && sentences.length > 0
...
function validateComments(comments: unknown): Comment[] {
   ...
   for (const comment of comments) {
      ...
      if (!c.id || !c.text || typeof c.text !== 'string' || c.text.trim().length === 0) {
         throw new Error('All comments must have valid id and non-empty text fields');
      }
   }
...
function processComments(commentsArray: Comment[], includeStats: boolean = false): CommentAnalysis[] {
...
function calculateSummary(results: CommentAnalysis[]): AnalysisSummary {
   ...
   const averageConfidence = results.reduce((sum, result) => sum + result.confidence, 0) / totalComments;
   const averageScore = results.reduce((sum, result) => sum + result.score, 0) / totalComments;

   // Calculate sentiment changes between original and analyzed
   const sentimentChanges = results.reduce((acc, result) => {
      if (result.originalSentiment !== result.sentiment) {
         acc.total++;

         // Check if sentiment improved or worsened
         const originalScore = result.originalSentiment === 'positive' ? 1 : result.originalSentiment === 'negative' ? -1 : 0;
         const newScore = result.sentiment === 'positive' ? 1 : result.sentiment === 'negative' ? -1 : 0;
