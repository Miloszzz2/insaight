...
export default function AnalysisPage() {
  ...
  const analysisSteps = [
    ...
    },
    {
      ...
      action: async (comments: Comment[]) => {
        const analyzedComments: AnalyzedCommentsApiResponse = await analyzeSentiment(comments)

        setAnalysisData((prev) => ({
          ...prev,
          comments: analyzedComments.data?.results || [],
          sentimentSummary: analyzedComments.data?.summary.sentimentDistribution || { positive: 0, neutral: 0, negative: 0 }
        }))
    ...
    },
    {
      ...
      icon: Film,
      action: async (comments: Comment[]) => {
        const categorizedData: CategorizationResult = await categorizeComments(comments)
        console.log(categorizedData)
        setAnalysisData((prev) => ({
          sentimentSummary: { positive: categorizedData.overallSummary.sentiment.positive, negative: categorizedData.overallSummary.sentiment.negative, neutral: categorizedData.overallSummary.sentiment.neutral },
  ...
  return (
    <SidebarProvider>
      <AnalysisSidebar categories={analysisData.categories} isAnalyzed={isAnalyzed} />
      <SidebarInset>
        {/* Header */}
          ...
          {!isAnalyzed && !isAnalyzing && !error && (
            ...
            <div className="max-w-4xl mx-auto text-center space-y-8">
              <div className="space-y-4">
                ...
                <h2 className="text-2xl font-bold text-gray-900">Ready to Analyze Your Comments</h2>
                <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                  Our AI will fetch and analyze comments from your video, categorize them by topics, and provide
                  sentiment analysis to help you understand your audience better.
                </p>
              </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="text-sm">Comment Categories</span>
                    </div>
                    <div className="flex items-center space-x-3">
          ...
          {isAnalyzing && (
            ...
            <div className="max-w-4xl mx-auto text-center space-y-8">
              <div className="space-y-4">
                ...
                <h2 className="text-2xl font-bold text-gray-900">Analyzing Your Comments</h2>
                <p className="text-lg text-gray-600">
                  Our AI is processing your comments. This may take a few minutes.
                </p>
              </div>
                    ...
                    {analysisSteps.map((step, index) => {
                      ...
                      return (
                        <div key={index} className="flex items-center space-x-3">
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center ${isCompleted ? "bg-green-100" : isActive ? "bg-violet-100" : "bg-gray-100"
                              }`}
                          >
                            {isCompleted ? (
                              ...
                              <IconComponent
                                className={`w-4 h-4 ${isUpcoming ? "text-gray-400" : "text-violet-600"}`}
                              />
                            )}
                          </div>
                          <span
            ...
            <>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-xl sm:text-2xl font-bold text-green-600 mb-2">
                      {Math.round((analysisData.sentimentSummary.positive / analysisData.comments.length) * 100)}%
                    </div>
                    <Progress
                      value={(analysisData.sentimentSummary.positive / analysisData.comments.length) * 100}
                      className="h-2 bg-green-100"
                    />
                    <p className="text-sm text-gray-600 mt-2">{analysisData.sentimentSummary.positive} comments</p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-xl sm:text-2xl font-bold text-gray-600 mb-2">
                      {Math.round((analysisData.sentimentSummary.neutral / analysisData.comments.length) * 100)}%
                    </div>
                    <Progress
                      value={(analysisData.sentimentSummary.neutral / analysisData.comments.length) * 100}
                      className="h-2 bg-gray-100"
                    />
                    <p className="text-sm text-gray-600 mt-2">{analysisData.sentimentSummary.neutral} comments</p>
                  </CardContent>
                </Card>
                ...
                <Card className="sm:col-span-2 lg:col-span-1">
                  ...
                  <CardContent>
                    <div className="text-xl sm:text-2xl font-bold text-red-600 mb-2">
                      {Math.round((analysisData.sentimentSummary.negative / analysisData.comments.length) * 100)}%
                    </div>
                    <Progress
                      value={(analysisData.sentimentSummary.negative / analysisData.comments.length) * 100}
                      className="h-2 bg-red-100"
                    />
                    <p className="text-sm text-gray-600 mt-2">{analysisData.sentimentSummary.negative} comments</p>
                  </CardContent>
                </Card>
              </div>
                ...
                <CardContent>
                  <div className="bg-violet-50 border border-violet-200 rounded-lg p-4">
                    <p className="text-sm sm:text-base text-gray-700">
                      {analysisData.aiSummary || "AI summary will appear here after analysis..."}
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Showing {analysisData.comments.length} analyzed comments</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="mb-4">
                      ...
                      >
                        <div className="flex items-start space-x-3">
                          <Avatar className="w-8 h-8 flex-shrink-0">
                            ...
                            <AvatarFallback>{comment.author_name[0] || "None"}</AvatarFallback>
                          </Avatar>
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1">
                              <span className="font-medium text-gray-900 truncate">{comment.author_name || "None"}</span>
                              <div className="flex items-center space-x-2">
                                {getSentimentIcon(comment.sentiment)}
                                <span className="text-sm text-gray-500">{comment.created_at}</span>
                              </div>
                            </div>
                            <p className="text-sm sm:text-base text-gray-700 mb-2">{comment.text}</p>
